...
/**
 * 유지칼로리 본문 — 숫자·계산 근거 배지·추천 목표·적용.
 *
 * 체중 탭 카드(TdeeCard)와 프로필 재계산 시트(TdeeRecalcSheet)가 **같은 숫자를 두 곳에서** 보여준다.
 * 따로 그리면 갈라진다 — 한쪽만 배지를 빼먹거나, 추천이 현재 목표와 같을 때의 문구가 달라진다(design D4).
 * 제목·껍데기(카드/시트)는 호출측이 갖고, 이 컴포넌트는 내용만 그린다.
 */
export function TdeeSummary({ tdee, onApplied }: { tdee: TdeeInfo; onApplied?: () => void }) {
  ...
  // 프로필·기록 부족으로 유지칼로리조차 못 낸 경우 — 숫자 대신 무엇을 더 하면 되는지 말한다
  if (tdee.maintenanceKcal == null) {
    return (
      <p className="mt-1 text-sm text-muted">
        체중과 식사를 꾸준히 기록하면 실제 데이터로 유지칼로리를 계산해드려요.
      </p>
    )
  }
  ...
  return (
    <>
      <div className="mt-1 flex items-center justify-between gap-2">
        ...
        {/* 숫자 옆에 근거를 붙인다 — 근거 없이 값만 바뀌면 그것을 믿을 이유가 없다 */}
        <span
