...
export function Button({
  ...
  // active:scale — 모바일에는 hover가 없어 누른 순간 아무 반응이 없었다. 살짝 눌리면 닿은 게 읽힌다
  return (
    <button
      {...props}
      className={`rounded-tile px-4 py-2 text-sm font-medium transition-transform active:scale-[0.98] touch-manipulation focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-ink ${styles} ${className}`}
    />
  )
}
