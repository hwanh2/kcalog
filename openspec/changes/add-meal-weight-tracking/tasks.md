...
- [ ] 1.1 Tailwind CSS 도입 — Vite 플러그인 설정, 디자인 토큰(색·간격·타이포) 정의, 전역 리셋
- [ ] 1.2 3탭 하단 네비게이션 셸 — 오늘/기록/프로필 탭, 현재 탭 강조, 인증 가드 하위 라우트 구성
- [ ] 1.3 기존 화면 편입 — 온보딩·프로필·홈을 셸·스타일에 맞게 이식 (기능 회귀 없이)
- [ ] 1.4 앱 셸 테스트 — 탭 이동, 온보딩 미완료 시 셸 미노출

## 2. 백엔드: 체중 (weight-tracking)

- [ ] 2.1 `POST /api/weights` — 오늘/지정일 체중 upsert(기존 네이티브 쿼리 재사용), 범위 검증(30~250)
- [ ] 2.2 `GET /api/weights?from=&to=` — 기간별 추이 조회(날짜 오름차순), 최신 체중 조회
- [ ] 2.3 체중 통합 테스트 — 오늘 기록·같은 날 upsert(1행 유지)·범위 밖 400·기간 조회
