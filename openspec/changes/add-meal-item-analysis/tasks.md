...
- [ ] 3.1 분석 응답 타입·API 갱신(items+box), 저장 요청을 items로
- [ ] 3.2 사진 미리보기 + 오버레이(박스+라벨) / 목록형 폴백 (신뢰도·박스 유효성으로 모드 결정)
- [ ] 3.3 음식별 편집·추가·삭제 + 합계 자동 재계산, 저장(`POST /api/meals`)
- [ ] 3.4 기록 탭 음식별 표시·수정(항목 편집) — RecordsPage 갱신
- [ ] 3.5 프론트 테스트 — 오버레이/목록 모드 분기, 항목 편집·추가·삭제·합계 재계산, 저장, 기록 수정
...
- [ ] 5.2 설계 문서(docs) 개정 — 음식별 분석·오버레이(사진 미저장) 반영, 이탈 사항 design.md 정리
