...
- [ ] 5.7 👤 Tailscale로 SSH가 되는 것을 확인한 **뒤에** 보안 그룹의 22번 규칙을 지운다
  - 순서를 바꾸면 서버에 잠긴다. 확인 전에는 지우지 않는다
...
- [ ] 7.1 👤 Tailscale 주소로 Grafana가 열리는지: 맥과 **아이폰 둘 다**
...
- [ ] 7.3 👤 Prometheus 대상 목록에서 백엔드가 `UP`인지
- [ ] 7.4 👤 Grafana에서 JVM 힙 그래프가 그려지는지
- [ ] 7.5 👤 백엔드를 잠시 멈춰 `up=0`이 기록되는지 (D3의 핵심 근거)
...
- [ ] 9.3 👤 외부 감시 2건 (D10)
  - 업타임 모니터: `api.kcalog.site/actuator/health`를 밖에서 찔러본다
  - 🔴 heartbeat 모니터: **모니터링 VM이 주기적으로 신호를 보낸다.** 찔러보는 방식이 아니다.
    그 VM은 SSH 말고 열린 포트가 없어(D9) 밖에서 닿을 수 없다. cron으로 5분마다 ping URL을 친다
- [ ] 9.4 👤 업타임 서비스 알림도 같은 슬랙 채널로
  - 경로를 하나로 모아야 장애 때 시간순으로 한 화면에서 읽힌다

### 10. 검증과 문서

- [ ] 10.1 👤 Grafana에서 `{app="kcalog", level="ERROR"}`로 에러만 걸리는지
- [ ] 10.2 👤 예외 로그를 열었을 때 **스택 트레이스가 통째로** 보이는지 (D5의 목적)
- [ ] 10.3 👤 요청 ID로 한 요청의 로그가 묶여 나오는지
- [ ] 10.4 👤 백엔드를 잠시 멈춰 **슬랙으로 알림이 실제로 오는지**
  - 알림은 만들었다고 되는 게 아니라 와야 된 것이다
- [ ] 10.5 👤 모니터링 VM을 잠시 멈춰 데드맨 스위치가 도는지
