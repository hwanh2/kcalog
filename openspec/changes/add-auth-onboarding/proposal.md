...
- 소셜 로그인: Spring Security + OAuth2 Client(카카오/구글) 기반 로그인/로그아웃
...
- `auth`: 카카오/구글 OAuth2 소셜 로그인, JWT access/refresh 토큰 발급·갱신·만료 처리, 로그아웃
...
- 외부 의존: 카카오/구글 OAuth 앱 등록 및 클라이언트 키 발급 필요 (환경 변수로 관리)
