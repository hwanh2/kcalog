...
- [x] 1.3 신호 계산 6~8개(달성일·평균vs목표·단백질미달·지방연속초과·탄수연속초과·성실도·체중추세·섭취vs유지) + 테스트
...
- [x] 3.2 `WeeklyReportService.get(memberId, weekStart)` — 일별 섭취 집계 → 달성·분포·신호·TDEE 조립
- [x] 3.3 `ReportController` `GET /api/reports/weekly?weekStart=`
- [x] 3.4 통합 테스트 — 달성·분포·TDEE·인사이트, 목표 없음/데이터 부족

## 4. 프론트엔드
- [x] 4.1 `api/report.ts` — 타입 + `getWeeklyReport(weekStart?)`
- [x] 4.2 리포트 페이지 — 주 이동(이전/다음), 달성 요약·탄단지 분포(SVG)·TDEE 스파크라인·인사이트 목록
- [x] 4.3 라우팅 `/report` ComingSoon → 리포트 페이지 교체 (AppShell 테스트 갱신)
- [x] 4.4 프론트 테스트(섹션 렌더·주 이동·데이터 부족)
