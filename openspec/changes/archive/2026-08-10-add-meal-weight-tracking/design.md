...
### D12. 마무리 결정 (2026-08-10) — 그룹 7 이관·8.1 생략 후 archive
`docs/2026-08-10-ai-diet-app-design.md`(재계획: 비동기 분석·사진 저장·영양 DB 매칭·학습형 수정·적응형 TDEE·리포트·AI PT·5탭 셸)로 로드맵이 전면 개편됨에 따라, 그룹 7(eval)은 모델 승격/강등 판단 시점의 별도 change로 이관(add-meal-item-analysis 4.1과 동일 결정), 8.1(E2E)은 식사 흐름 실사용 검증 완료 + 현행 화면 전면 개편 예정을 근거로 생략하고 이 change를 archive한다. 구현된 기능(체중·식사 CRUD·분석·대시보드)은 새 로드맵의 기반으로 유지된다.
