...
- [x] 5.1 사진 입력·리사이즈 — `<input capture>` 카메라/갤러리, Canvas 긴 변 1024px·JPEG 80% 압축
- [x] 5.2 분석 호출·로딩 — analyze 멀티파트 전송, 분석 중 표시, 실패 시 수동 입력 폴백
- [x] 5.3 확인·수정 화면 — 영양값 표시·수정, 끼니 구분(시간대 기본값), 저장(`POST /api/meals`)
- [x] 5.4 수동 입력 — 사진 없이 영양값 직접 입력 저장 경로
- [x] 5.5 기록 조회·수정·삭제 UI — 기록 탭 날짜별 목록, 개별 수정·삭제
- [x] 5.6 식사 흐름 테스트 — 분석→확인→저장, 수동 입력, 수정·삭제

## 6. 프론트: 대시보드·체중 (daily-dashboard, weight-tracking)

- [x] 6.1 백엔드 `GET /api/dashboard?date=` — 해당일 meal 집계(총계·잔여·탄단지 비율·타임라인) + 통합 테스트
- [x] 6.2 오늘 탭 대시보드 — 잔여 칼로리·탄단지·타임라인 표시, TanStack Query 도입(저장 후 무효화로 자동 갱신)
- [x] 6.3 체중 입력 UI — 기록 탭에서 체중 입력(오늘/지정일)
- [x] 6.4 체중 추이 그래프 — 기간별 조회 결과 시각화
- [x] 6.5 대시보드·체중 화면 테스트 — 잔여 계산 표시, 저장 후 갱신, 체중 입력·추이

## 7. 평가 세트 (meal-analysis-eval)

- [ ] 7.1 `eval/` 다양한 음식 사진 20~30장 + 기대값(JSON) 수집·정리 *(보류 — 2026-08-10 재계획으로 이관, add-meal-item-analysis 4.1과 함께 모델 승격/강등 판단 시점에 착수. design.md D12 참조)*
- [ ] 7.2 채점 스크립트 — 각 사진 분석→기대값 대비 오차(MAPE 등) 집계, 모델·프롬프트 비교 출력 (수동 실행, CI 비포함) *(보류 — 7.1과 동일)*
- [ ] 7.3 GPT-5.4 mini 기준 실측 → nano 강등/5.5 승격 판단, 결과를 design.md에 기록 *(보류 — 7.1과 동일)*

## 8. 마무리

- [ ] 8.1 E2E 수동 검증 — 사진 촬영→분석→확인·수정→저장→오늘 대시보드 갱신, 체중 입력→추이, 기록 수정·삭제 *(생략 — 식사 흐름은 사용자 실사용 검증 완료, 나머지 화면은 2026-08-10 재계획으로 전면 개편 예정이라 현행 화면 E2E는 생략. design.md D12 참조)*
- [x] 8.2 `.env.example`·README(로컬 실행 시 OpenAI 키) 갱신, 설계 문서와의 이탈 사항 design.md 반영
