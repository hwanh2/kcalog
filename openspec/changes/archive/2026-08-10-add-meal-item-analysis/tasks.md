...
- [ ] 4.1 채점에 음식별 지표 추가 — 항목 수·이름 매칭·항목별 영양 오차·박스 IoU  *(보류 — 모델 승격/강등 판단 시점에 별도 착수, 박스 IoU는 선택. design.md D6 결정 참조)*
- [x] 4.2 초기 실측 → 오버레이 유지 vs 목록형 기본 결정, design.md에 기록 *(실사용 스모크로 박스 품질 OK 확인 → 오버레이 유지, D6에 기록)*

## 5. 마무리

- [x] 5.1 E2E 수동 검증 — 실제 사진 다항목 분석→오버레이/목록→편집→저장→기록 음식별 조회·수정 *(사용자 실사용 확인 완료)*
