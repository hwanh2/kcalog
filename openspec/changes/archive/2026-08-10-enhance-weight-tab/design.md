...
- **UI(사용자 목업 반영)**: 체중 기록 카드를 오렌지 그라데이션 히어로 카드(흰 큰 숫자 + −0.1/＋0.1 버튼 + 크림색 저장 + 🔥 streak 칩)로, 목표 달성 예상은 밝은 카드(진행바 brand→carb·예상일 강조)로 구성. 상단 날짜 선택 캘린더 제거 — 탭은 **오늘** 기록 전용(과거일 수정은 후속). BMI는 목업에 없어 탭 UI엔 노출 안 함(백엔드 응답엔 유지). 변화 지표는 직전 기록 대비이며, 두 점이 실제 하루 차이면 **"어제보다"**, 아니면 **"직전 대비"**로 라벨링(결측일 보간 없음).
- **projectedDate 기준 = 최신 기록일(anchorDate)** (D4의 `today+days`가 아님): 외삽은 추세의 마지막 점에서 하므로 `anchorDate + days`가 자연스럽다. 프론트 D-day는 `오늘→projectedDate`로 재계산하되, 최신 기록이 과거면 음수가 될 수 있어 **양수일 때만 노출**하고, 도달(weeks=0·잔여<0.1kg)은 별도 "도달" 문구로 처리한다.
