...
## 7. 음식기록 탭 뼈대 (프론트)
- [x] 7.1 `mealDefaults` — 끼니 5개 라벨, `defaultMealType` 규칙 변경(00~05·21~24 야식, 간식 자동선택 제외) + 테스트
- [x] 7.2 `RecordsPage` 재작성 — 끼니 세그먼트(배지 = 그 끼니 기록 수) + 선택 끼니 기록 표 + 끼니 합계
- [x] 7.3 기록 카드 — 사진 썸네일(없으면 식기 아이콘)·끼니/시각·음식 요약·수량 표시·매크로 칩·삭제
- [x] 7.4 라우팅 정리 — `/meals/new` 제거, `AppShell` FAB이 `/records?camera=1`로 이동 + 카메라 즉시 열기
- [x] 7.5 프론트 테스트 — 세그먼트 전환·합계·빈 상태·FAB 동작
- [x] 7.6 프론트 "오늘"도 05시 경계로 (`todayServiceDate`) — 홈 대시보드 포함 (tasks 추가)

## 8. 자주먹는 탭 (프론트)
- [x] 8.1 `api/food.ts` — 카탈로그·즐겨찾기 통합 조회
- [x] 8.2 검색 매칭 (TDD) — 정규화 → 이름(정확·앞부분·부분) → 별칭(정확·부분) → 결과가 없을 때만 음절 bigram 자카드(임계 0.3)
- [x] 8.3 목록 — ★ 토글·이름/기준량·탄단지·kcal·`[+]` (2열 그리드 대신 1열, 이모지 제거)
- [x] 8.4 수량 바텀시트 — 증감(단위별 눈금)·영양 재계산 표시·"{끼니}에 기록하기"
- [x] 8.5 검색 결과 없음 → "'{검색어}' 직접 추가하기"
- [x] 8.6 프론트 테스트 — 검색 매칭·수량 변경 시 재계산·담기 후 저장 요청

## 9. 즐겨찾기 탭 (프론트)
- [x] 9.1 목록·★ 해제·`[+]` 담기(수량 시트 공용)
- [x] 9.2 저장 시트 — 이름·수량·단위·영양 + "AI 분석에도 반영" 체크박스
- [x] 9.3 "직접 만들기" 진입점 + 빈 상태 일러스트
- [x] 9.4 프론트 테스트 — 저장·삭제·체크박스 전달

## 10. AI로 기록 탭 (프론트)
- [x] 10.1 입력 화면 — 사진 선택(선택)·설명 입력(선택)·둘 다 없으면 제출 불가
- [x] 10.2 분석 진행 표시 — 사진 스캔 애니메이션 + 결과 자리 스켈레톤(`AnalyzingView`)
- [x] 10.3 결과 시트 — `PhotoOverlay`(배지형)·`AnalysisSummary`·`AnalyzedItemList`·`ItemEditSheet` 재사용, 사진 없으면 목록형
- [x] 10.4 항목별 수량 표시, 항목 ★로 즐겨찾기 저장
- [x] 10.5 재분석 — 설명 입력·수정값 있으면 덮어쓰기 경고·2회 상한 안내·실패 시 직전 결과 복원
- [x] 10.6 "AI 없이 직접 입력하기" → 수동 입력 시트(8.5와 공용)
- [x] 10.7 실패·429·NO_FOOD 폴백 유지
- [x] 10.8 프론트 테스트 — 입력 조합·FAB 진입 시 AI 탭 자동 선택

## 10-1. PR #34 리뷰 대응
- [x] 10-1.1 **체크됐지만 검증이 없던 태스크의 테스트 보강**(🔴) — 즐겨찾기 저장/해제·`rememberForAnalysis` 전달·직접 입력(`AddFoodPanel.test`), 입력 3조합·429·NO_FOOD·실패 폴백(`AiRecordPanel.test`), 재분석 경고·상한·복원·항목 ★ 저장(`AnalysisResultSheet.test`). 삭제한 `MealRecordPage.test`의 회귀 커버리지를 되찾았다
- [x] 10-1.2 오버레이 모드를 분석 시점에 고정 — "+ 음식 추가"로 배지가 민무늬로 뒤집히던 회귀 수정
- [x] 10-1.3 정규화 함수 통일 — `normalizeKey` 제거, `search.normalizeName` 하나로
- [x] 10-1.4 담기 직전 "오늘" 재계산 + 탭 복귀 시 동기화 — 05시 경계를 넘겨 열어둬도 어제로 새지 않게
- [x] 10-1.5 반올림·수량 표기 헬퍼를 `lib/number.ts`로 통합
- [x] 10-1.6 `eatenAtFor` 과거 분기가 현재 미연결임을 주석으로 명시
- [x] 10-1.7 검색 순서 문구 정정(tasks 8.2·design D12)

## 11. 마무리
- [x] 11.1 `./gradlew test` · `npm run build` · `npm test` · `openspec validate --strict` 통과
- [x] 11.2 design.md에 구현 이탈 반영
