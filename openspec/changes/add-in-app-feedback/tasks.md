...
- [x] 2.2 `FeedbackSheet` — 입력·글자 수·보낸 뒤 확인 (보내는 정보 안내는 2.4a에서 뺐다)
...
## 5. PR #45 리뷰 대응 (🔴 0 · nit 5 + 외 2)

- [x] 5.1 tasks 2.2에 남아 있던 "보내는 정보 안내" 잔재 정리 — 2.4a에서 뺀 것과 어긋났다
- [x] 5.2 **메일 전용 실행기** `mailExecutor` + `@Async("mailExecutor")` — 이름 없는 `@Async`는
      `TaskExecutor` 빈이 둘이라 기본 실행기로 떨어졌다. 나중에 `taskExecutor` 빈이 생기면 조용히 옮겨간다 (D12)
- [x] 5.3 **상한 예외를 `RateLimitException`으로 통합** — 429 핸들러가 셋으로 복제돼 있었다.
      세 도메인 모두 429 통합 테스트가 있어 합친 뒤에도 통과 (D13)
- [x] 5.4 메일 헤더 CRLF 제거 — 실제 악용 경로는 없지만(값 출처가 카카오 프로필) 한 줄로 막고 테스트로 남긴다
- [x] 5.5 상한이 원자적이지 않은 것은 **의도한 차이**임을 design에 기록 — 돈이 나가는 상한(분석·코치)과
      달리 의견은 관대함을 스펙이 요구한다 (D3)
- [x] 5.6 `FeedbackResponse` 이름은 그대로 — `{Domain}Response` 컨벤션을 하나만 예외로 두지 않는다 (D14)

## 6. 마무리

- [ ] 6.1 **GitHub Secrets 등록** — 2단계 인증 → 앱 비밀번호 발급 → `MAIL_HOST=smtp.gmail.com`,
      `MAIL_USERNAME`, `MAIL_PASSWORD`(앱 비밀번호), `FEEDBACK_TO`. 넣기 전까지는 저장만 되고 발송은 건너뛴다
- [ ] 6.2 **배포 후 실제 메일 도착 확인** — 앱에서 의견을 하나 보내보고 받은편지함과
