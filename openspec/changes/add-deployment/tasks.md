...
- [x] 0.1 👤 도메인 `kcalog.site` 구매
- [x] 0.2 👤 DNS 레코드 — `kcalog.site` → Vercel(안내대로 A/CNAME), `api.kcalog.site` → EC2 Elastic IP(A 레코드)
...
- [x] 0.3 👤 EC2 인스턴스 생성 — **`t3.small`**(x86, 2GB), Ubuntu 24.04 LTS, 루트 볼륨 **30GiB**(gp3, 암호화), **Elastic IP 할당**, 보안 그룹 인바운드 **22·80·443**
  ...
  - **SSH(22)를 특정 IP로 제한하지 않는다.** GitHub Actions 러너가 SSH로 배포하는데 러너 IP는 매 실행마다 바뀌고, GitHub이 공개하는 대역은 수천 개 CIDR이라 보안 그룹 규칙 한도(기본 60)에 넣을 수 없다. 대신 키 인증만 허용(Ubuntu 기본값 — 비밀번호 로그인 비활성)하고 배포 전용 계정을 관리자와 분리해 방어한다
  - 기본 8GiB로는 부족하다 — 이미지(회당 약 800MB, 7일 보관) + Postgres 데이터 + 로그가 한 디스크에 쌓이고, 디스크가 차면 DB가 멈춘다. 프리 티어가 30GiB까지 무료다
- [x] 0.4 👤 VM 초기 세팅 — Docker + compose 플러그인 설치, 배포 전용 사용자 생성 후 docker 그룹에 추가
- [x] 0.5 👤 배포용 SSH 키쌍 생성(이 용도 전용) — 공개키는 VM의 배포 사용자에, 개인키는 GitHub Secret에
- [x] 0.6 👤 Cloudflare R2 버킷 2개 생성 — 사진용, 백업용. S3 호환 액세스 키 발급
- [x] 0.7 👤 카카오 개발자 콘솔 — Redirect URI에 `https://api.kcalog.site/login/oauth2/code/kakao` 추가, 사이트 도메인 등록
- [x] 0.8 👤 OpenAI 비용 천장 확보 (완전 공개라 유일한 비용 천장 — design Risks 참고)
  - 실제로는 월 한도가 아니라 **선불 잔액 + 자동 충전 OFF**로 막았다. 잔액이 바닥나면 호출이 실패할 뿐 요금이 더 나가지 않는다
- [x] 0.9 👤 AWS Budgets 예산 알림 설정
- [ ] 0.10 👤 `release` 브랜치 생성 + 보호 규칙 — CI 통과 필수, 직접 푸시 제한(핫픽스 예외는 본인만)
  - 브랜치는 생성·운영 중이나 **보호 규칙이 아직 없다**(`branches/release/protection` 404). 실수로 밀린 커밋이 그대로 운영 배포가 된다
- [x] 0.11 👤 ghcr 패키지는 **private 유지**. VM이 pull할 수 있도록 PAT 발급 → `GHCR_PULL_TOKEN`
...
- [x] 0.13 👤 Vercel 프로젝트 연결 — 루트 디렉터리 **`frontend`**, 환경변수 `VITE_API_BASE_URL=https://api.kcalog.site`(Production), **Production Branch를 `release`로 지정**(Git 연동이 프론트 운영 배포를 담당한다 — 워크플로우는 백엔드만 다룬다), 도메인 `kcalog.site`·`www` 연결
...
- [x] 5.2 👤 VM cron에 일 1회 등록
- [ ] 5.3 **복구 검증** — 백업 파일을 빈 DB에 복원해 실제로 살아나는지 확인 (백업은 복구해 봐야 백업이다)

## 6. 첫 배포·검증

- [x] 6.1 main → `release` 머지로 첫 배포 실행
- [x] 6.2 HTTPS 인증서 발급 확인 (`https://api.kcalog.site/actuator/health`)
- [x] 6.3 Flyway V1~V17이 빈 운영 DB에 정상 적용됐는지 확인
- [x] 6.4 **카카오 로그인 전체 흐름** — 로그인 → 콜백 → 프론트 복귀 → 새로고침 후 세션 유지(refresh 쿠키가 same-site로 전달되는지)
  - iOS 홈 화면 실행(standalone) 상태에서도 확인 — 로그인 후 앱을 완전히 종료했다 다시 열어도 세션이 유지된다
...
- [x] 6.7 딥링크 확인 — `https://kcalog.site/weight` 직접 진입·새로고침
