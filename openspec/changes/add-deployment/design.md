...
### D9. 운영 DB 접속 — 루프백 공개 + SSH 터널

GUI 클라이언트(DataGrip 등)로 운영 DB를 직접 보고 고칠 수 있게 한다. 운영 데이터를 눈으로 확인할 길이
`docker exec psql`뿐이면 스키마를 훑거나 몇 행을 고치는 일이 매번 SSH 세션 안의 텍스트 작업이 된다.

**`127.0.0.1:5432:5432`로만 공개한다.** 포트를 아예 열지 않으면 SSH 터널의 끝에 듣는 포트가 없어
클라이언트가 붙을 수 없고, 접두사 없이 열면 0.0.0.0에 바인딩된다.

0.0.0.0 바인딩이 위험한 이유는 **방어선이 하나로 줄기 때문**이다. VM 안의 방화벽(UFW 등)은 도움이 되지
않는다 — 도커가 iptables 앞단에 자기 규칙을 넣어 UFW의 거부 규칙을 지나친다. 실제로 막아주는 건 EC2
보안 그룹뿐인데(VM 밖에서 걸리므로 도커가 우회하지 못한다), 그러면 규칙 하나가 잘못 열리는 순간
운영 DB가 그대로 노출된다. 접두사는 그 실수를 무해하게 만든다.

대안으로 컨테이너 IP에 직접 터널을 뚫는 방법(`ssh -L <포트>:<컨테이너IP>:5432`)이 있어 구성 변경이
필요 없지만, 컨테이너 IP가 배포마다 바뀌어 매번 다시 확인해야 한다. 상시 쓸 경로로는 맞지 않는다.

**읽기 전용 계정을 따로 두지 않는다.** 쓰기까지 하겠다는 판단이고(운영자 1인), 계정을 나눠도
쓰기 계정을 함께 쓰는 이상 방어가 되지 않는다. 실수의 최종 방어선은 야간 백업(D5)이다.

## Risks / Trade-offs

- **[운영 DB에 GUI로 쓰기 접속]** D9의 대가. 오타 하나가 운영 데이터를 되돌릴 수 없게 바꿀 수 있고, GUI는 확인 절차 없이 커밋되는 경로가 많다. 되돌릴 방법은 야간 백업뿐이라 **최대 24시간치가 날아갈 수 있다** — 큰 작업 전에는 `backup.sh`를 손으로 한 번 돌린다. 5432가 루프백에만 열리므로 외부 노출은 늘지 않는다(SSH가 여전히 유일한 관문).
