...
"사진 한 장으로 10초 안에 식사가 기록되는 AI 식단·체중 관리 앱" (범용 식단 관리 — 2026-08-07 한식 특화에서 전환). 실서비스 출시 지향 + Spring 학습 목적 사이드 프로젝트. 서비스명은 **kcalog로 확정**(도메인 `kcalog.site`, 2026-08-12) — 패키지명 `com.kcalog`을 그대로 쓴다. 2026-08-19부터 **사용자에게 보이는 표기는 "칼로그"** 한글이다(홈 화면 아이콘 이름, 앱 헤더, 랜딩, OG 카드). 도메인, 패키지명, 파일명은 영문 kcalog 그대로다.
