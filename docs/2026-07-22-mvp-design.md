...
- **분석 프롬프트**: "한국 음식 분석가"에서 범용 "음식 영양 분석 전문가"로 변경 — 한식·양식·중식·일식 등 어떤 음식이든 총량(칼로리·탄단지)을 추정. 한식도 잘 다루되 한식만 전제하지 않는다.
- **제품 정의·전략 개정**: 아래 본문 §1의 "한식 사진…", "한식 특화 분석을 훅으로", §1 차별화의 "한식 맥락", §6 "프롬프트 설계(한식 특화)"·"한국 음식 영양 분석가"는 모두 **범용 식단 관리**로 대체해 읽는다. 제품 한 줄 정의는 **"사진 한 장으로 10초 안에 식사가 기록되는 체중 관리 앱"**.
- **차별화**: 한식 반찬 분리 분석은 더 이상 핵심 차별화가 아니다. 차별화는 기록 마찰 최소화(사진→확인→저장 속도)와 이후 대화형 AI 코치(B)에 둔다.
- **eval 세트**: "한식 사진 20~30장"이 아니라 **다양한 음식 사진 20~30장 + 기대값**으로 구성해 범용 정확도를 측정한다.

### 2026-08-05: 식사 분석 AI를 OpenAI GPT-5.4 mini로 변경 (Claude Haiku 4.5에서)

- **결정**: 사진 분석 모델을 **GPT-5.4 mini**로 시작하고, `eval/` 세트(한식 사진 20~30장 + 기대값) 실측 결과로 승격/강등을 판단한다 — 충분하면 nano로 강등(1/4 가격), 부족하면 5.5로 승격 시험. 모델명·provider는 설정값으로 분리해 교체가 코드 변경 없이 가능하게 한다.
- **근거**: ① 음식 사진 영양 추정 실측 연구(2025)에서 ChatGPT·Claude 계열이 오차 ~36%로 동급, Gemini는 64~110%로 열세 — 품질 근거가 있는 계열 중 저렴한 티어 선택. ② 장당 비용 mini ~$0.0018(월 9,000장 기준 ~$16)로 일 20회 제한과 함께 충분히 저렴. ③ 오차의 지배 요인은 모델 지능이 아닌 사진 한 장의 정보 한계(포션·국물·조리법)라 플래그십(5.5, 7배 비용)의 개선폭이 불확실하고, 분석 3~8초 목표에는 경량 모델의 속도가 유리.
- 본문 §3·§6의 "Claude Vision API" 서술은 이 개정으로 대체된다 (2026-07-28 개정의 Haiku 4.5 결정 포함).
