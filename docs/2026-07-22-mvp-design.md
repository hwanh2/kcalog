...
### 2026-07-28: 1차 범위 축소 (체중·식단 우선)

핵심 루프를 먼저 출시하기 위해 1차 범위를 **체중 기록 + 식단 기록(AI 분석 포함) + 최소 대시보드**로 좁힌다. 상세 결정:

- **1차 제외 → 이후 단계로**: 운동 기록, 주간 AI 리포트, 음식별 아이템 분석(meal_item), 사진 보관(스토리지)
- **식단 기록 단위 단순화**: 사진 1장 = 기록 1건, 총량 4개 값(칼로리·탄·단·지)만 기록. `meal_item` 테이블은 1차 스키마에서 제외. 끼니 구분(meal_type)은 유지하고 시간대 기반 기본값을 자동 제안
- **사진은 저장하지 않음**: 프론트 → 백엔드 분석 API 멀티파트 전송 → Claude Vision 호출 → 결과 반환 후 폐기. `photo_key` 컬럼·S3/R2·presigned URL·로컬 MinIO 전부 1차에서 제외. 사진 보관은 이후 별도 change(컬럼 추가는 additive 마이그레이션)
- **AI 모델**: Haiku 4.5로 시작(총량 추정은 경량 모델로 충분 가정), 모델명은 설정값으로 분리해 교체 용이하게. `eval/` 평가 세트로 정확도 검증 후 부족하면 Sonnet 승격. 일일 분석 20회 제한 유지
- **화면 3탭**: 오늘(잔여 칼로리·탄단지 합계 + 타임라인 + 카메라) / 기록(달력 조회 + 체중 입력·추이 그래프) / 프로필(목표 수정·계정). 리포트 탭은 기능 추가 시 복원
- **트레이드오프 명시**: 사진 단위 총량 단순화로 "한식 반찬 분리 분석" 차별화 포인트는 1차에서 빠진다. 제품 한 줄 정의("사진 한 장으로 10초 기록")는 유지되며, 차별화는 이후 meal_item 구조 도입 시 살린다
- **진행 순서**: ① `add-auth-onboarding` 그룹 4~6 마무리(로그인·온보딩 프론트, 카카오 문서화·E2E) → ② 새 change(식단·체중·대시보드) propose

아래 본문 중 이 개정과 충돌하는 내용(섹션 2의 운동·리포트, 섹션 5의 meal_item·photo_key·workout_log·weekly_report, 섹션 4·6의 presigned 업로드·아이템 분리 분석, 섹션 7의 4탭)은 **이후 단계의 목표 설계**로 읽는다. 1차 범위는 이 개정 이력이 우선한다.
